...
| 10. Layered Architecture | `completed` | Control, orchestration, execution, policy, context, and supervision responsibilities are all materially present across `src/hermit/runtime/` and `src/hermit/kernel/`. | `Core` / `Governed` / `Verifiable` | Exact target layer names are not yet the literal module layout; several runtime-era boundaries still remain. |
