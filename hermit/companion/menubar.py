...
if rumps is not None:  # pragma: no branch - class only exists when dependency is available
    ...
    class HermitMenuApp(rumps.App):  # pragma: no cover - macOS UI exercised manually
        def __init__(
            ...
            self.webhook_enabled_item = rumps.MenuItem(
        ...
        def _show_result(
            self, title: str, action: str, fn, *args, busy_label: str | None = None, **kwargs
        ) -> None:
        ...
        def _open_readme(self, _sender) -> None:
            try:
                target = readme_path()
                if target.exists():
                    open_in_textedit(target)
        ...
        def _open_wiki(self, _sender) -> None:
            try:
