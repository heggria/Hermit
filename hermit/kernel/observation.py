...
def normalize_observation_progress(value: Any) -> ObservationProgress | None:
    ...
    if not summary and not phase:
        return None
    return ObservationProgress.from_dict(value)
...
class ObservationTicket:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "status_tool_input": dict(self.status_tool_input or {}) if self.status_tool_input else None,
            "cancel_tool_name": self.cancel_tool_name,
            "cancel_tool_input": dict(self.cancel_tool_input or {}) if self.cancel_tool_input else None,
            "progress": dict(self.progress or {}) if self.progress else None,
            "progress_summary": dict(self.progress_summary or {}) if self.progress_summary else None,
...
def normalize_observation_ticket(value: Any) -> ObservationTicket | None:
    raw = value
    if isinstance(value, dict) and _OBSERVATION_ENVELOPE_KEY in value:
        raw = value.get(_OBSERVATION_ENVELOPE_KEY)
    ...
    if not required.issubset(raw.keys()):
        return None
    ticket = ObservationTicket.from_dict(raw)
