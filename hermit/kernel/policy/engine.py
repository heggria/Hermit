    ...
    def evaluate(
        ...
        *,
        ...
        if isinstance(tool_or_request, ActionRequest):
            ...
            request = self.build_action_request(tool_or_request, payload or {}, attempt_ctx=attempt_ctx)
        outcomes = evaluate_rules(request)
        decision = merge_outcomes(outcomes, action_class=request.action_class, default_risk=request.risk_hint)
        if decision.approval_packet is not None:
            ...
            packet.setdefault("risk_level", decision.obligations.approval_risk_level or decision.risk_level)
