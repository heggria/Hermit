...
class KernelStoreRecordMixin:
    ...
    def _approval_from_row(self, row: sqlite3.Row) -> ApprovalRecord:
        return ApprovalRecord(
            ...
            fallback_contract_refs=list(_json_loads(row["fallback_contract_refs_json"] or "[]")),
    ...
    def _execution_contract_from_row(self, row: sqlite3.Row) -> ExecutionContractRecord:
        return ExecutionContractRecord(
            ...
            proposed_action_refs=list(_json_loads(row["proposed_action_refs_json"])),
            expected_effects=list(_json_loads(row["expected_effects_json"])),
            success_criteria=dict(_json_loads(row["success_criteria_json"] or "{}")),
            evidence_case_ref=row["evidence_case_ref"],
            authorization_plan_ref=row["authorization_plan_ref"],
            reversibility_class=str(row["reversibility_class"] or "reversible"),
            required_receipt_classes=list(_json_loads(row["required_receipt_classes_json"])),
            drift_budget=dict(_json_loads(row["drift_budget_json"] or "{}")),
            expiry_at=row["expiry_at"],
            status=str(row["status"]),
            fallback_contract_refs=list(_json_loads(row["fallback_contract_refs_json"] or "[]")),
            operator_summary=row["operator_summary"],
            risk_budget=dict(_json_loads(row["risk_budget_json"] or "{}")),
            expected_artifact_shape=dict(_json_loads(row["expected_artifact_shape_json"] or "{}")),
            contract_version=int(row["contract_version"] or 1),
            action_contract_refs=list(_json_loads(row["action_contract_refs_json"] or "[]")),
            state_witness_ref=row["state_witness_ref"],
    ...
    def _evidence_case_from_row(self, row: sqlite3.Row) -> EvidenceCaseRecord:
        return EvidenceCaseRecord(
            ...
            support_refs=list(_json_loads(row["support_refs_json"])),
            contradiction_refs=list(_json_loads(row["contradiction_refs_json"])),
            freshness_window=dict(_json_loads(row["freshness_window_json"] or "{}")),
            sufficiency_score=float(row["sufficiency_score"] or 0.0),
            drift_sensitivity=str(row["drift_sensitivity"] or "medium"),
            unresolved_gaps=list(_json_loads(row["unresolved_gaps_json"] or "[]")),
            status=str(row["status"]),
            witness_refs=list(_json_loads(row["witness_refs_json"] or "[]")),
            invalidates_refs=list(_json_loads(row["invalidates_refs_json"] or "[]")),
            last_checked_at=row["last_checked_at"],
            confidence_interval=dict(_json_loads(row["confidence_interval_json"] or "{}")),
    ...
    def _authorization_plan_from_row(self, row: sqlite3.Row) -> AuthorizationPlanRecord:
        return AuthorizationPlanRecord(
            ...
            requested_action_classes=list(
                _json_loads(row["requested_action_classes_json"] or "[]")
            ),
            required_decision_refs=list(_json_loads(row["required_decision_refs_json"] or "[]")),
            approval_route=str(row["approval_route"] or "none"),
            witness_requirements=list(_json_loads(row["witness_requirements_json"] or "[]")),
            proposed_grant_shape=dict(_json_loads(row["proposed_grant_shape_json"] or "{}")),
            downgrade_options=list(_json_loads(row["downgrade_options_json"] or "[]")),
            current_gaps=list(_json_loads(row["current_gaps_json"] or "[]")),
            status=str(row["status"]),
            estimated_authority_cost=float(row["estimated_authority_cost"])
            if row["estimated_authority_cost"] is not None
            else None,
            expiry_constraints=dict(_json_loads(row["expiry_constraints_json"] or "{}")),
            revalidation_rules=dict(_json_loads(row["revalidation_rules_json"] or "{}")),
            ...
            proposed_lease_shape=dict(_json_loads(row["proposed_lease_shape_json"] or "{}")),
            created_at=float(row["created_at"]),
            updated_at=float(row["updated_at"]),
        )
    ...
    def _reconciliation_from_row(self, row: sqlite3.Row) -> ReconciliationRecord:
        return ReconciliationRecord(
            ...
            step_attempt_id=str(row["step_attempt_id"]),
            contract_ref=str(row["contract_ref"]),
            receipt_refs=list(_json_loads(row["receipt_refs_json"] or "[]")),
            observed_output_refs=list(_json_loads(row["observed_output_refs_json"] or "[]")),
            ...
            invalidated_belief_refs=list(_json_loads(row["invalidated_belief_refs_json"] or "[]")),
            superseded_memory_refs=list(_json_loads(row["superseded_memory_refs_json"] or "[]")),
            promoted_template_ref=row["promoted_template_ref"],
            promoted_memory_refs=list(_json_loads(row["promoted_memory_refs_json"] or "[]")),
            operator_summary=row["operator_summary"],
