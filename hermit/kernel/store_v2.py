...
class KernelV2StoreMixin:
    ...
    def update_execution_contract(
        ...
        *,
        ...
        operator_summary: str | None | object = _UNSET,
        superseded_by_contract_id: str | None | object = _UNSET,
    ) -> None:
        contract = self.get_execution_contract(contract_id)
        if contract is None:
            return
        ...
        payload = {
            ...
            if evidence_case_ref is _UNSET
    ...
    def update_evidence_case(
        ...
        *,
        status: str | object = _UNSET,
        contradiction_refs: list[str] | object = _UNSET,
        unresolved_gaps: list[str] | object = _UNSET,
        operator_summary: str | None | object = _UNSET,
        sufficiency_score: float | object = _UNSET,
        last_checked_at: float | object = _UNSET,
    ) -> None:
        record = self.get_evidence_case(evidence_case_id)
        if record is None:
            return
        updated_at = time.time()
        payload = {
            "status": record.status if status is _UNSET else status,
            "contradiction_refs": record.contradiction_refs
            if contradiction_refs is _UNSET
            else list(contradiction_refs),
            "unresolved_gaps": record.unresolved_gaps
            if unresolved_gaps is _UNSET
            else list(unresolved_gaps),
            "operator_summary": record.operator_summary
            if operator_summary is _UNSET
            else operator_summary,
            "sufficiency_score": record.sufficiency_score
            if sufficiency_score is _UNSET
            else float(sufficiency_score),
            "last_checked_at": record.last_checked_at
            if last_checked_at is _UNSET
        ...
        with self._lock, self._conn:
            self._conn.execute(
                ...
                """,
                (
                    json.dumps(payload["contradiction_refs"], ensure_ascii=False),
                    payload["sufficiency_score"],
                    json.dumps(payload["unresolved_gaps"], ensure_ascii=False),
                    payload["status"],
                    payload["operator_summary"],
    ...
    def update_authorization_plan(
        ...
        *,
        status: str | object = _UNSET,
        current_gaps: list[str] | object = _UNSET,
        operator_packet_ref: str | None | object = _UNSET,
    ) -> None:
        record = self.get_authorization_plan(authorization_plan_id)
        if record is None:
            return
        ...
        payload = {
            "status": record.status if status is _UNSET else status,
            "current_gaps": record.current_gaps if current_gaps is _UNSET else list(current_gaps),
            "operator_packet_ref": record.operator_packet_ref
            if operator_packet_ref is _UNSET
            else operator_packet_ref,
        }
        with self._lock, self._conn:
            self._conn.execute(
                """
                ...
                (
                    payload["status"],
                    json.dumps(payload["current_gaps"], ensure_ascii=False),
                    payload["operator_packet_ref"],
                    updated_at,
                    authorization_plan_id,
                ),
            )
