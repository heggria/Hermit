        ...
        prepared.append(
            {
                **message,
                "content": [
                    prepare_image_block(block, max_inline_image_bytes=max_inline_image_bytes)
...
def prepare_image_block(
    ...
    *,
    ...
    media_type = _normalize_media_type(str(source.get("media_type", "")).strip())
...
def compress_image_bytes(
    ...
    *,
    ...
    for compressor in (_compress_with_pillow, _compress_with_sips):
        try:
            candidate_media_type, candidate_bytes = compressor(image_bytes, normalized_media_type, max_bytes)
...
def _compress_with_pillow(
    ...
    for _ in range(_MAX_COMPRESSION_PASSES):
        for candidate_media_type, candidate_bytes in _encode_candidates_with_pillow(current, media_type):
            if len(candidate_bytes) < len(best_bytes):
                best_media_type = candidate_media_type
                best_bytes = candidate_bytes
