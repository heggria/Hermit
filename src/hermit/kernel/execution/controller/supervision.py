...
class SupervisionService:
    ...
    def build_task_case(self, task_id: str) -> dict[str, Any]:
        ...
        proof = cast(dict[str, Any], cached["proof"])
        claims = cached.get("claims") or task_claim_status(self.store, task_id, proof_summary=proof)
        latest_receipt = cast(dict[str, Any] | None, proof.get("latest_receipt"))
        latest_decision = cast(dict[str, Any] | None, proof.get("latest_decision"))
        latest_capability_grant = cast(dict[str, Any] | None, proof.get("latest_capability_grant"))
        ...
        target_paths = list(
            (latest_capability_grant or {})
            .get("constraints", cast(dict[str, Any], {}))
            .get("target_paths", [])
        )
    ...
    def _build_ingress_observability(self, task: TaskRecord | None) -> dict[str, Any]:
        ...
        focus_entry: dict[str, Any] | None = next(
            (
                cast(dict[str, Any], entry)
                for entry in list(conversation_projection.get("open_tasks", []) or [])
                if str(cast(dict[str, Any], entry).get("task_id", "") or "") == focus_task_id
            ),
            None,
        )
        return {
