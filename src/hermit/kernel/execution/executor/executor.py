...
class ToolExecutor:
    ...
    def _record_reconciliation(
        ...
        *,
        ...
        if not self.receipt_service.verify_receipt_bundle_signature(receipt_id):
            ...
            self.store.append_event(
                event_type="receipt.signature_verification_failed",
                entity_type="receipt",
                ...
                payload={
                    "receipt_id": receipt_id,
                    "step_attempt_id": attempt_ctx.step_attempt_id,
                    "warning": "Receipt signature verification failed — possible tampering",
                },
            )
    ...
    def _supersede_attempt_for_drift(
        ...
        *,
        ...
        drift_retry_count = int(current.context.get("drift_retry_count", 0) or 0)
        if drift_retry_count >= _MAX_DRIFT_RETRIES:
            self.store.update_step_attempt(
                attempt_ctx.step_attempt_id,
                status="failed",
                waiting_reason=f"Drift retry limit exceeded ({_MAX_DRIFT_RETRIES}): {drift_reason}",
            )
            self.store.update_step(attempt_ctx.step_id, status="failed")
            self.store.update_task_status(attempt_ctx.task_id, "failed")
            return ToolExecutionResult(
                model_content=(
                    f"[Drift Retry Limit] Exceeded max drift retries"
                    f" ({_MAX_DRIFT_RETRIES}) for {drift_reason}."
                ),
                raw_result={"drift_reason": drift_reason, "retries": drift_retry_count},
                blocked=True,
                denied=True,
                result_code="drift_retry_limit_exceeded",
                execution_status="failed",
                state_applied=True,
            )
        now = time.time()
        ...
        successor = self.store.create_step_attempt(
            ...
            context={
                **dict(current.context),
                ...
                "drift_retry_count": drift_retry_count + 1,
