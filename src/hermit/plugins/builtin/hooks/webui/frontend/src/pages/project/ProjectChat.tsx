// Project Chat tab -- reuses the Chat page pattern with project context.
...
export default function ProjectChat() {
  ...
  const handleSend = useCallback(() => {
    ...
    send({ type: 'message', text, context: { program_id: programId } });
  ...
  }, [input, connected, isResponding, send, addMessage, programId]);
        ...
        <div className="mx-auto max-w-3xl">
          <div className="relative flex items-end gap-2 rounded-2xl border border-border/60 bg-card px-4 py-2.5 shadow-sm transition-shadow focus-within:shadow-md focus-within:border-primary/30">
            <textarea
              ...
              placeholder={
                connected ? t('chat.placeholder') : t('chat.placeholderDisabled')
              }
              disabled={!connected}
              rows={1}
