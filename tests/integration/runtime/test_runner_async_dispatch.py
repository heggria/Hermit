...
from hermit.kernel.task.services import controller as controller_module
from hermit.runtime.control.runner import runner as runner_module
...
def test_runner_start_stop_background_services_and_wake(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "hermit.runtime.control.runner.runner.ObservationService", FakeObservationService
    )
    monkeypatch.setattr(
        "hermit.kernel.execution.coordination.dispatch.KernelDispatchService", FakeDispatchService
    )
...
def test_runnerdispatch_control_action_covers_kernel_introspection_paths(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "hermit.kernel.execution.controller.supervision.SupervisionService", FakeSupervisionService
    )
    monkeypatch.setattr("hermit.kernel.verification.proofs.proofs.ProofService", FakeProofService)
    monkeypatch.setattr(
        "hermit.kernel.verification.rollbacks.rollbacks.RollbackService", FakeRollbackService
    )
    monkeypatch.setattr(
        "hermit.kernel.task.projections.projections.ProjectionService", FakeProjectionService
    )
...
def test_runner_close_session_and_resume_attempt_cover_finalize_and_blocked_fallback() -> None:
    runner, agent, _session_manager, pm, controller, _store = _make_runner()
...
def test_runner_records_scheduler_delivery_metadata(tmp_path: Path) -> None:
    runner, _agent, _session_manager, _pm, _controller, store = _make_runner(tmp_path)
    task_ctx = SimpleNamespace(
        task_id="task-1",
        step_attempt_id="attempt-1",
        source_channel="scheduler",
