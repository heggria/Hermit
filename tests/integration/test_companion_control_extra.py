...
def test_matching_process_pids_uses_exact_base_dir(tmp_path: Path) -> None:
    ...
    process_table = "\n".join(
        [
            f"101 UV=1 HERMIT_BASE_DIR={dev_base} PATH=/tmp python -m hermit.surfaces.cli.main serve --adapter feishu",
            f"202 UV=1 HERMIT_BASE_DIR={prod_base} PATH=/tmp python -m hermit.surfaces.cli.main serve --adapter feishu",
            f"303 UV=1 HERMIT_BASE_DIR={prod_base} PATH=/tmp python -m hermit.apps.companion.menubar --adapter feishu",
        ]
    )

    assert control.watch_pid_path("feishu", base_dir=prod_base) == prod_base / "watch-feishu.pid"
    assert control.matching_process_pids(
        "-m hermit.surfaces.cli.main serve --adapter feishu",
        base_dir=prod_base,
        process_table=process_table,
    ) == [202]
    assert control.matching_process_pids(
        "-m hermit.apps.companion.menubar --adapter feishu",
        base_dir=prod_base,
        process_table=process_table,
    ) == [303]
...
def test_command_prefix_and_project_reference_fallbacks(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(control.shutil, "which", lambda name: None)
    assert control.command_prefix() == [str(fake_python), "-m", "hermit.surfaces.cli.main"]
...
def test_run_command_and_service_status_darwin_paths(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "hermit.surfaces.cli.autostart._plist_path", lambda adapter: tmp_path / "launch.plist"
    )
    monkeypatch.setattr("hermit.surfaces.cli.autostart._is_loaded", lambda adapter: True)
...
def test_start_stop_reload_and_extract_preflight_paths(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        control, "command_prefix", lambda: ["python", "-m", "hermit.surfaces.cli.main"]
    )
    monkeypatch.setattr(
        control.subprocess,
        "Popen",
