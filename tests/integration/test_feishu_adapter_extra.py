...
from hermit.kernel.policy.approvals.approval_copy import ApprovalCopy, ApprovalSection
from hermit.kernel.task.services.controller import TaskController
from hermit.plugins.builtin.adapters.feishu.adapter import FeishuAdapter
from hermit.plugins.builtin.adapters.feishu.reply import make_tool_step
...
def _force_feishu_locale(monkeypatch):
    ...
    monkeypatch.setattr(
        "hermit.plugins.builtin.adapters.feishu.adapter.log.warning", lambda *args, **kwargs: None
    )
...
def test_feishu_adapter_present_task_result_patches_existing_card_for_blocked_approval(
    ...
    monkeypatch.setattr(
        "hermit.plugins.builtin.adapters.feishu.adapter.patch_card",
...
def test_feishu_adapter_patch_task_topic_updates_signature_once(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr(
        "hermit.plugins.builtin.adapters.feishu.adapter.ProjectionService",
...
def test_feishu_adapter_reissues_only_feishu_oc_approval_cards(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "hermit.plugins.builtin.adapters.feishu.adapter.send_card",
...
def test_feishu_adapter_process_message_reports_dispatch_failure_without_progress_card(
    ...
    monkeypatch.setattr(
        "hermit.plugins.builtin.adapters.feishu.adapter.send_ack", lambda *_args, **_kwargs: None
    )
    monkeypatch.setattr(
        "hermit.plugins.builtin.adapters.feishu.adapter.send_text_reply",
...
def test_feishu_adapter_process_message_note_appended_refreshes_topic(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "hermit.plugins.builtin.adapters.feishu.adapter.send_done",
