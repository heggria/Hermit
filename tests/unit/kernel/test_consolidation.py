...
def test_run_consolidation_returns_report(fast_store: KernelStore, monkeypatch) -> None:
    """Basic consolidation run returns a ConsolidationReport."""
    _patch_topic_tokens(monkeypatch)
    svc = ConsolidationService()
    _create_memory(fast_store, claim_text="test memory for consolidation")

    report = svc.run_consolidation(fast_store)

    assert isinstance(report, ConsolidationReport)
    assert report.consolidated_at > 0


def test_dedup_merges_similar_memories(fast_store: KernelStore, monkeypatch) -> None:
    """Two nearly identical memories result in one being invalidated."""
    _patch_topic_tokens(monkeypatch)
    svc = ConsolidationService()
    _create_memory(fast_store, claim_text="ruff is the standard formatter", confidence=0.7)
    _create_memory(fast_store, claim_text="ruff is the standard formatter", confidence=0.9)
    ...
    invalidated = fast_store.list_memory_records(status="invalidated")
    assert len(invalidated) >= 1


def test_dedup_keeps_higher_confidence(fast_store: KernelStore, monkeypatch) -> None:
    """The winner of dedup has the higher confidence."""
    _patch_topic_tokens(monkeypatch)
    svc = ConsolidationService()
    low = _create_memory(fast_store, claim_text="ruff is the standard formatter", confidence=0.5)
    high = _create_memory(fast_store, claim_text="ruff is the standard formatter", confidence=0.95)

    svc.run_consolidation(fast_store)

    low_record = fast_store.get_memory_record(low.memory_id)
    high_record = fast_store.get_memory_record(high.memory_id)
...
def test_strengthen_pass_ignores_non_qualifying(fast_store: KernelStore, monkeypatch) -> None:
...
def test_decay_pass_runs(fast_store: KernelStore, monkeypatch) -> None:
    """Decay pass executes during consolidation without error."""
    _patch_topic_tokens(monkeypatch)
    svc = ConsolidationService()
    _create_memory(fast_store, claim_text="memory subject to decay")
...
def test_consolidation_report_fields(fast_store: KernelStore, monkeypatch) -> None:
    """All ConsolidationReport fields are populated after a run."""
    _patch_topic_tokens(monkeypatch)
    svc = ConsolidationService()
    _create_memory(fast_store, claim_text="some memory")
...
def test_empty_store_consolidation(fast_store: KernelStore, monkeypatch) -> None:
...
def test_strengthen_pass_boosts_frequently_referenced(fast_store: KernelStore, monkeypatch) -> None:
    ...
    updated = fast_store.get_memory_record(m.memory_id)
...
def test_strengthen_pass_skips_episode_index(fast_store: KernelStore, monkeypatch) -> None:
...
def test_strengthen_pass_skips_influence_link(fast_store: KernelStore, monkeypatch) -> None:
...
def test_strengthen_pass_skips_non_int_reference_count(
    fast_store: KernelStore, monkeypatch
) -> None:
...
def test_strengthen_pass_respects_confidence_cap(fast_store: KernelStore, monkeypatch) -> None:
...
def test_dedup_skips_already_seen_inner(fast_store: KernelStore, monkeypatch) -> None:
    """When three identical memories exist, the inner loop skips already-seen entries."""
    _patch_topic_tokens(monkeypatch)
    svc = ConsolidationService()
    m1 = _create_memory(fast_store, claim_text="ruff is the standard formatter", confidence=0.9)
    _create_memory(fast_store, claim_text="ruff is the standard formatter", confidence=0.7)
    _create_memory(fast_store, claim_text="ruff is the standard formatter", confidence=0.6)

    report = svc.run_consolidation(fast_store)

    # At least 2 should be merged (invalidated)
    assert report.merged_count >= 2
    winner = fast_store.get_memory_record(m1.memory_id)
    assert winner is not None
    assert winner.status == "active"


def test_reflect_pass_promotes_insights(fast_store: KernelStore, monkeypatch) -> None:
...
def test_reflect_pass_with_mocked_services(fast_store: KernelStore, monkeypatch) -> None:
    ...
    fake_record = _create_memory(fast_store, claim_text="placeholder")
    ...
    mock_reflect.promote_insight.assert_called_once_with(insight, fast_store)


def test_reflect_pass_skips_unpromoted_insights(fast_store: KernelStore, monkeypatch) -> None:
...
def test_anti_pattern_pass_inverts_pitfalls(fast_store: KernelStore, monkeypatch) -> None:
    ...
    m = _create_memory(fast_store, claim_text="a failing pattern")
    mock_anti = MagicMock()
    ...
    pitfall_record = _create_memory(
        fast_store, claim_text="PITFALL: a failing pattern", memory_kind="pitfall_warning"
    )
    ...
    mock_anti.invert_to_pitfall.assert_called_once_with(m.memory_id, fast_store)


def test_anti_pattern_pass_skips_failed_inversion(fast_store: KernelStore, monkeypatch) -> None:
...
def test_text_similarity_with_embedding_service(fast_store: KernelStore, monkeypatch) -> None:
    """When embedding service is provided, it is used for similarity."""
    _patch_topic_tokens(monkeypatch)
    from unittest.mock import MagicMock

    mock_embeddings = MagicMock()
    mock_embeddings.embed.side_effect = lambda text: [1.0, 0.0] if "high" in text else [0.0, 1.0]
    ...
    _create_memory(fast_store, claim_text="high confidence fact A", confidence=0.9)
    _create_memory(fast_store, claim_text="high confidence fact B", confidence=0.7)
...
def test_text_similarity_embedding_fallback_on_error(fast_store: KernelStore, monkeypatch) -> None:
...
def test_text_similarity_empty_tokens(fast_store: KernelStore, monkeypatch) -> None:
    """Empty claim text returns 0.0 similarity (no merge)."""
    _patch_topic_tokens(monkeypatch)
    svc = ConsolidationService()

    m1 = _create_memory(fast_store, claim_text="", confidence=0.8)
    m2 = _create_memory(fast_store, claim_text="", confidence=0.7)

    svc.run_consolidation(fast_store)

    # Empty tokens → similarity 0.0 → no merge
    r1 = fast_store.get_memory_record(m1.memory_id)
    r2 = fast_store.get_memory_record(m2.memory_id)
    assert r1 is not None and r1.status == "active"
    assert r2 is not None and r2.status == "active"


def test_dedup_different_memories_not_merged(fast_store: KernelStore, monkeypatch) -> None:
