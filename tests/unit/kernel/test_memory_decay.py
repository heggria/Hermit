...
def test_fresh_memory_within_half_ttl(fast_store: KernelStore) -> None:
    """Memory at 10% TTL consumed should be classified as FRESH."""
    service = MemoryDecayService()
    now = time.time()
    record = _create_memory(fast_store, retention_class="user_preference")
    # user_preference TTL = 365 days; put memory at 10% of TTL
    ttl_seconds = 365 * 24 * 60 * 60
    age_seconds = ttl_seconds * 0.10
    _age_memory(fast_store, record.memory_id, now - age_seconds)

    refreshed = fast_store.get_memory_record(record.memory_id)
...
def test_aging_memory_between_half_and_three_quarter_ttl(fast_store: KernelStore) -> None:
...
def test_stale_memory_between_three_quarter_and_ninety_pct(
...
def test_expired_memory_beyond_ninety_pct(fast_store: KernelStore) -> None:
...
def test_decay_sweep_updates_freshness_class(fast_store: KernelStore) -> None:
    ...
    report = service.run_decay_sweep(fast_store, now=now)
...
def test_quarantine_moves_to_quarantined_status(fast_store: KernelStore) -> None:
    """quarantine() should set status to 'quarantined' and record the reason."""
    service = MemoryDecayService()
    record = _create_memory(fast_store)

    result = service.quarantine(fast_store, record.memory_id, reason="test decay")
...
def test_revive_restores_quarantined_memory(fast_store: KernelStore) -> None:
    """revive() should reset a quarantined memory to active with fresh evidence."""
    service = MemoryDecayService()
    record = _create_memory(fast_store)

    # First quarantine the memory
    service.quarantine(fast_store, record.memory_id, reason="stale")

    # Then revive it with new evidence
    result = service.revive(
        fast_store, record.memory_id, new_evidence_refs=["ev-new-1", "ev-new-2"]
    )
...
def test_sweep_skips_audit_retention(fast_store: KernelStore) -> None:
    ...
    _age_memory(fast_store, audit_record.memory_id, now - ttl_seconds * 0.95)
    ...
    _age_memory(fast_store, normal_record.memory_id, now - vol_ttl * 0.95)
...
def test_sweep_collects_quarantine_candidates(fast_store: KernelStore) -> None:
    """Expired memories should appear in the report's quarantine_candidates list."""
    service = MemoryDecayService()
    now = time.time()

    record = _create_memory(fast_store, retention_class="volatile_fact")
    # volatile_fact TTL = 1 day; age to 95% → EXPIRED
    ttl_seconds = 24 * 60 * 60
    _age_memory(fast_store, record.memory_id, now - ttl_seconds * 0.95)
...
def test_evaluate_freshness_with_last_accessed_at(fast_store: KernelStore) -> None:
...
def test_quarantine_returns_false_for_nonexistent(fast_store: KernelStore) -> None:
    """quarantine() returns False when memory does not exist or is not active."""
    service = MemoryDecayService()

    # Non-existent memory (line 147: record is None)
    result = service.quarantine(fast_store, "nonexistent-id", reason="test")
    assert result is False


def test_quarantine_returns_false_for_non_active(fast_store: KernelStore) -> None:
    """quarantine() returns False when memory is not in active status."""
    service = MemoryDecayService()
    record = _create_memory(fast_store)
    fast_store.update_memory_record(record.memory_id, status="invalidated")

    # Line 147: record.status != "active"
    result = service.quarantine(fast_store, record.memory_id, reason="test")
    assert result is False


def test_revive_returns_false_for_nonexistent(fast_store: KernelStore) -> None:
    """revive() returns False when memory does not exist."""
    service = MemoryDecayService()

    # Line 166: record is None
    result = service.revive(fast_store, "nonexistent-id", new_evidence_refs=["ev-1"])
    assert result is False


def test_revive_returns_false_for_active_memory(fast_store: KernelStore) -> None:
    """revive() returns False when memory is active (not quarantined)."""
    service = MemoryDecayService()
    record = _create_memory(fast_store)

    # Line 166: record.status != "quarantined"
    result = service.revive(fast_store, record.memory_id, new_evidence_refs=["ev-1"])
    assert result is False


def test_effective_ttl_uses_explicit_expires_at(fast_store: KernelStore) -> None:
    ...
    conn = fast_store._get_conn()  # type: ignore[attr-defined]
    conn.execute(
