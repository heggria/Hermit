...
def test_extract_entities_uses_pattern(fast_store: KernelStore) -> None:
    """Extracting 'project uses Python' produces a triple with predicate 'uses'."""
    svc = MemoryGraphService()
    record = _create_memory(fast_store, claim_text="project uses Python")
...
def test_extract_entities_is_pattern(fast_store: KernelStore) -> None:
    """Extracting 'ruff is the formatter' produces a triple with predicate 'is'."""
    svc = MemoryGraphService()
    record = _create_memory(fast_store, claim_text="ruff is the formatter")
    triples = svc.extract_entities(record)
...
def test_extract_entities_no_match(fast_store: KernelStore) -> None:
    """Plain text without entity patterns produces no triples."""
    svc = MemoryGraphService()
    record = _create_memory(fast_store, claim_text="hello world")
    triples = svc.extract_entities(record)

    assert triples == []


def test_build_edges_temporal_sequence(fast_store: KernelStore) -> None:
    """Two memories in the same task created close together get a temporal_sequence edge."""
    ensure_graph_schema(fast_store)
    svc = MemoryGraphService()
    now = time.time()

    m1 = _create_memory(fast_store, task_id="t1", claim_text="first observation")
    m2 = _create_memory(fast_store, task_id="t1", claim_text="second observation")
    # Ensure both are within 1 hour of each other
    _age_memory(fast_store, m1.memory_id, now - 60)
    _age_memory(fast_store, m2.memory_id, now)

    edges = svc.build_edges(m1.memory_id, fast_store)
...
def test_build_edges_same_entity(fast_store: KernelStore) -> None:
    """Two memories sharing an extracted entity get a same_entity edge."""
    ensure_graph_schema(fast_store)
    svc = MemoryGraphService()

    m1 = _create_memory(fast_store, claim_text="project uses Python")
    m2 = _create_memory(fast_store, claim_text="project uses ruff")

    # Extract and store triples for both
    t1 = svc.extract_entities(m1)
    t2 = svc.extract_entities(m2)
    svc.store_triples(t1, fast_store)
    svc.store_triples(t2, fast_store)
...
def test_multi_hop_retrieve_single_hop(fast_store: KernelStore) -> None:
    """BFS finds a 1-hop neighbor from a seed memory."""
    ensure_graph_schema(fast_store)
    svc = MemoryGraphService()
    now = time.time()

    m1 = _create_memory(fast_store, task_id="t1", claim_text="seed memory")
    m2 = _create_memory(fast_store, task_id="t1", claim_text="neighbor memory")
    _age_memory(fast_store, m1.memory_id, now)
    _age_memory(fast_store, m2.memory_id, now)

    # Build edges to create the graph link
    svc.build_edges(m1.memory_id, fast_store)

    results = svc.multi_hop_retrieve(
        "test query", fast_store, seed_memory_ids=[m1.memory_id], max_hops=1
    )
...
def test_multi_hop_retrieve_two_hops(fast_store: KernelStore) -> None:
    ...
    m1 = _create_memory(fast_store, task_id="t1", claim_text="project uses Python")
    m2 = _create_memory(fast_store, task_id="t2", claim_text="Python requires ruff")
    m3 = _create_memory(fast_store, task_id="t3", claim_text="ruff needs configuration")

    # Extract and store triples so entity edges can be built
    for m in [m1, m2, m3]:
        triples = svc.extract_entities(m)
        svc.store_triples(triples, fast_store)

    # Build edges from each memory
    svc.build_edges(m1.memory_id, fast_store)
    svc.build_edges(m2.memory_id, fast_store)

    results = svc.multi_hop_retrieve(
        "test query", fast_store, seed_memory_ids=[m1.memory_id], max_hops=2
    )
...
def test_auto_link_by_topic(fast_store: KernelStore) -> None:
    ...
    m1 = _create_memory(
        fast_store, claim_text="Python ruff formatter is mandatory for linting checks"
    )
    _create_memory(fast_store, claim_text="ruff formatter handles Python code formatting")

    edges = svc.auto_link(m1.memory_id, fast_store)

    topic_edges = [e for e in edges if e.relation_type == "related_topic"]
    assert len(topic_edges) >= 1


def test_build_edges_missing_memory_returns_empty(fast_store: KernelStore) -> None:
    """build_edges returns [] when the memory_id doesn't exist (line 93)."""
    ensure_graph_schema(fast_store)
    svc = MemoryGraphService()
    edges = svc.build_edges("nonexistent-memory-id", fast_store)
    assert edges == []


def test_multi_hop_retrieve_empty_seeds_returns_empty(fast_store: KernelStore) -> None:
    """multi_hop_retrieve returns [] when seed_memory_ids is empty (line 150)."""
    ensure_graph_schema(fast_store)
    svc = MemoryGraphService()
    results = svc.multi_hop_retrieve("query", fast_store, seed_memory_ids=[])
    assert results == []

    results_none = svc.multi_hop_retrieve("query", fast_store, seed_memory_ids=None)
    assert results_none == []


def test_auto_link_missing_memory_returns_empty(fast_store: KernelStore) -> None:
    """auto_link returns [] when the memory doesn't exist (line 202)."""
    ensure_graph_schema(fast_store)
    svc = MemoryGraphService()
    edges = svc.auto_link("nonexistent-id", fast_store)
    assert edges == []


def test_auto_link_with_embeddings(fast_store: KernelStore) -> None:
    """auto_link uses embedding service when available (lines 207-224)."""
    ensure_graph_schema(fast_store)

    m1 = _create_memory(fast_store, claim_text="Python uses pytest for testing")
    m2 = _create_memory(fast_store, claim_text="Testing with pytest is great")
...
def test_auto_link_embedding_below_threshold_skipped(fast_store: KernelStore) -> None:
    """auto_link skips embedding results below _SEMANTIC_THRESHOLD (lines 214-215)."""
    ensure_graph_schema(fast_store)

    m1 = _create_memory(fast_store, claim_text="Python uses pytest for testing")
    _create_memory(fast_store, claim_text="Unrelated content about cooking")
...
def test_auto_link_embedding_exception_fallback(fast_store: KernelStore) -> None:
    """auto_link falls back to topic linking when embeddings raise (lines 225-226)."""
    ensure_graph_schema(fast_store)

    m1 = _create_memory(fast_store, claim_text="Python ruff formatter is mandatory for linting")
    _create_memory(fast_store, claim_text="ruff formatter handles Python code formatting")
...
def test_auto_link_by_topic_empty_tokens_returns_empty(fast_store: KernelStore) -> None:
    ...
    m1 = _create_memory(fast_store, claim_text="the")
    svc = MemoryGraphService()
    edges = svc.auto_link(m1.memory_id, fast_store)
...
def test_auto_link_by_topic_skips_episode_index(fast_store: KernelStore) -> None:
    ...
    for edge in edges:
        target = fast_store.get_memory_record(edge.to_memory_id)
        if target is not None:
            assert target.memory_kind != "episode_index"


def test_auto_link_by_topic_skips_empty_other_tokens(fast_store: KernelStore) -> None:
    ...
    _create_memory(fast_store, claim_text="x y z")
