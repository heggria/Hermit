...
def test_feishu_doc_tools_cover_success_and_validation(monkeypatch: pytest.MonkeyPatch) -> None:
    client = _ns(
        docx=_ns(
            v1=_ns(
                document=_ns(
                    create=lambda req: _Resp(data=_ns(document=_ns(document_id="doc-1"))),
                    raw_content=lambda req: _Resp(data=_ns(content="Hello Feishu")),
                ),
                document_block_children=_ns(create=lambda req: _Resp(data=_ns())),
            )
        ),
        drive=_ns(
            v1=_ns(
    ...
    doc_create = feishu_tools._build_doc_create_tool().handler(
        {"title": " Plan ", "folder_token": "fld"}
    )
    ...
    assert feishu_tools._build_doc_append_tool().handler(
...
def test_feishu_doc_create_and_read_cover_api_and_meta_failures(
    ...
    client = _ns(
        ...
        drive=_ns(
            v1=_ns(
                meta=_ns(batch_query=lambda req: (_ for _ in ()).throw(RuntimeError("skip meta")))
            )
        ),
    )
    monkeypatch.setattr(feishu_tools, "build_lark_client", lambda: client)
...
def test_feishu_wiki_tools_cover_failure_paths(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert (
        feishu_tools._build_wiki_create_tool().handler({"space_id": "sp-1", "title": "Roadmap"})[
            "code"
        ]
        == 91403
    )
...
def test_feishu_message_and_bitable_tools_cover_happy_paths(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    client = _ns(
        im=_ns(v1=_ns(message=_ns(create=lambda req: _Resp(data=_ns(message_id="msg-1"))))),
        bitable=_ns(
            v1=_ns(
                app_table_record=_ns(
                    ...
                    create=lambda req: _Resp(data=_ns(record=_ns(record_id="rec-2"))),
                )
            )
        ),
    )
...
def test_feishu_message_and_bitable_tools_cover_failure_paths(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    client = _ns(
        im=_ns(
            v1=_ns(message=_ns(create=lambda req: _Resp(ok=False, code=91403, msg="forbidden")))
        ),
        bitable=_ns(
            v1=_ns(
                app_table_record=_ns(
    ...
    assert (
        feishu_tools._build_send_message_tool().handler({"receive_id": "x", "text": "hi"})["code"]
        == 91403
    )
    assert (
        feishu_tools._build_bitable_query_tool().handler({"app_token": "app", "table_id": "tbl"})[
            "code"
        ]
        == 91403
    )
    assert (
        feishu_tools._build_bitable_add_tool().handler(
            {"app_token": "app", "table_id": "tbl", "fields": {"A": 1}}
        )["code"]
        == 91403
    )
...
def test_feishu_sheet_and_calendar_tools_cover_success_and_api_errors(
    ...
    assert feishu_tools._build_calendar_create_tool().handler(
...
def test_feishu_sheet_and_calendar_tools_cover_success_variants_and_exceptions(
    ...
    client = _ns(
        ...
        calendar=_ns(
            v4=_ns(
                calendar_event=_ns(create=lambda req: _Resp(ok=False, code=91403, msg="forbidden"))
            )
        ),
    )
    monkeypatch.setattr(feishu_tools, "build_lark_client", lambda: client)
