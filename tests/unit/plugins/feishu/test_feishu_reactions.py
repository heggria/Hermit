...
from hermit.plugins.builtin.adapters.feishu import hooks as feishu_hooks
from hermit.plugins.builtin.adapters.feishu import reaction as feishu_reaction
...
def test_dispatch_result_sends_card_when_card_is_preferred(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "hermit.plugins.builtin.adapters.feishu.reply.should_use_card", lambda text: True
    )
    monkeypatch.setattr(
        "hermit.plugins.builtin.adapters.feishu.reply.build_result_card",
        lambda text: {"text": text},
    )
    monkeypatch.setattr(
        "hermit.plugins.builtin.adapters.feishu.reply.send_card",
...
def test_dispatch_result_sends_text_for_failures_when_card_not_needed(
    ...
    monkeypatch.setattr(feishu_hooks, "build_lark_client", lambda settings=None: object())
    monkeypatch.setattr(
        "hermit.plugins.builtin.adapters.feishu.reply.should_use_card", lambda text: False
    )
    monkeypatch.setattr(
        "hermit.plugins.builtin.adapters.feishu.reply.send_text_message",
