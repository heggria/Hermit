"""Tests for Telegram plugin hooks."""

from __future__ import annotations

from unittest.mock import AsyncMock, MagicMock, patch

from hermit.plugins.builtin.adapters.telegram.hooks import (
    _on_dispatch_result,
    _on_post_run,
    register,
)
...
def test_dispatch_result_no_chat_id():
...
def test_dispatch_result_invalid_chat_id():
    result = _on_dispatch_result(source="test", notify={"telegram_chat_id": "not-a-number"})
    assert result is None


@patch("hermit.plugins.builtin.adapters.telegram.hooks.get_active_adapter")
def test_dispatch_result_adapter_not_active(mock_get):
    mock_get.return_value = None
    result = _on_dispatch_result(
        source="test", title="Job", result_text="done", notify={"telegram_chat_id": "123"}
    )
...
@patch("hermit.plugins.builtin.adapters.telegram.hooks.get_active_adapter")
def test_dispatch_result_adapter_no_application(mock_get):
    adapter = MagicMock()
...
@patch("hermit.plugins.builtin.adapters.telegram.reply.send_message", new_callable=AsyncMock)
@patch("hermit.plugins.builtin.adapters.telegram.hooks.get_active_adapter")
def test_dispatch_result_success(mock_get, mock_send):
    adapter = MagicMock()
    adapter.application.bot = MagicMock()
    mock_get.return_value = adapter
...
def test_dispatch_result_exception(mock_get):
    ...
    result = _on_dispatch_result(source="test", notify={"telegram_chat_id": "123"})
    assert result is not None
    assert result["status"] == "failure"
...
def test_on_post_run_with_adapter(mock_get):
    ...
    # Should return without error — POST_RUN is a no-op for inline reply adapter
